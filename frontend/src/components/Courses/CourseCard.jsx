...
export default function CourseCard({ course }) {
  ...
  const { isInCart, addCourseToCart, isPending: isCartPending } = useCart()
  const { user } = useAuth()
  ...
  const ownsCourse = Boolean(courseId && user?.enrolledCourses?.some((entry) => String(entry?._id ?? entry) === courseId))
  ...
  return (
    <article className="ccard">
      ...
      {detailHref ? (
        <Link to={detailHref} className="ccard-media-link" aria-label={`View ${course.title}`} tabIndex={-1}>
          <div className="ccard-media">
            <CourseThumbnail course={course} alt={`${course.title} thumbnail`} preferPageImage preserveImageRatio />
          </div>
        </Link>
      ) : (
        <div className="ccard-media">
      ...
      <div className="ccard-body">
        ...
        {/* Clickable title navigates to course detail */}
        {detailHref ? (
          <Link to={detailHref} className="ccard-title-link">
        ...
        <div className="ccard-actions">
          {ownsCourse ? (
            <Link to={`/learn/${courseId}`} className="ccard-action-btn ccard-action-continue">
              Continue Learning
            </Link>
          ) : isFree ? (
            detailHref && (
              <Link to={detailHref} className="ccard-action-btn ccard-action-enroll">
                Enroll Now
              </Link>
            )
          ) : (
            <>
              <button
                className={`ccard-action-btn ccard-action-cart${inCart ? ' in-cart' : ''}`}
                onClick={() => !inCart && addCourseToCart(course)}
                disabled={isCartPending(course) || inCart}
                aria-label={inCart ? 'Already in cart' : 'Add to cart'}
              >
                {inCart ? 'In Cart ✓' : 'Add to Cart'}
              </button>
              {detailHref && (
                <Link to={detailHref} className="ccard-action-btn ccard-action-buy">
                  Buy Now
                </Link>
              )}
            </>
