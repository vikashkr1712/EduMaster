...
export default function SettingsPage() {
  ...
  // The backend profile endpoint only accepts name and email (strict schema).
  // Username / phone / bio stay local until their fields exist in the API.
  const saveProfile = async () => {
    ...
    const hasUnsupportedChanges = ['username', 'phone', 'bio'].some((key) => {
      const current = user?.[key] || (key === 'username' ? (user?.name || '').toLowerCase().replace(/\s+/g, '') : '')
      return form[key] !== current
    })

    if (Object.keys(payload).length === 0) {
      if (hasUnsupportedChanges) {
        info('Username, phone number, and bio are coming soon and cannot be saved yet.')
      } else {
    ...
    try {
      ...
      await refreshUser()
      setEditing(false)
      success(hasUnsupportedChanges
        ? 'Name and email updated. Username, phone number, and bio are coming soon.'
      ...
      <DashboardContent>
        ...
        >
          ...
          <p className="settings-hint">
            Only name and email are saved to your account for now — other fields are coming soon.
          </p>
        ...
        >
          <div className="settings-password-list">
            <PasswordInput id="settings-current-pass" label="Current Password" placeholder="Enter current password" autoComplete="current-password" />
            <PasswordInput id="settings-new-pass" label="New Password" placeholder="Enter new password" autoComplete="new-password" />
            <PasswordInput id="settings-confirm-pass" label="Confirm New Password" placeholder="Confirm new password" autoComplete="new-password" />
          </div>
          <p className="settings-hint">
            Password change is coming soon — this section is not connected yet.
          </p>
          <div className="settings-actions">
            <button
              type="button"
        ...
        >
          <div className="settings-row">
            ...
            <select className="settings-select" defaultValue="light" aria-label="Theme mode">
        ...
        >
          <div className="settings-rows">
            <ToggleSwitch
              ...
              onChange={(v) => setNotifications((n) => ({ ...n, course: v }))}
            />
            <ToggleSwitch
              icon={<MailIcon />}
            ...
            <ToggleSwitch
              ...
              onChange={(v) => setNotifications((n) => ({ ...n, offers: v }))}
            />
          </div>
        </SettingsCard>
